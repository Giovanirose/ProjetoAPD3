...
public class AtualizarContaController extends AbstractController {
    ...
    public void execute() {
        try {
            ...
            if(conexaoEstabelecida){                
                try{
                    result = dao.atualizar(conta);
                }catch(BancoDaoException ex){
                    ex.printStackTrace();
                }
            }
            this.setReturnPage("/atualizarConta.jsp");
            this.getRequest().setAttribute("atualizar_conta", result);
        } catch (Exception ex) {
